...
export const GET = withUser(async (user, _req: Request, ctx: Ctx) => {
  ...
  return ok({ conversation, messages: await listMessages(user.id, id) });
});

export const PATCH = withUser(async (user, req: Request, ctx: Ctx) => {
  const { id } = await ctx.params;
  const input = await parseBody(req, schema);
  const conversation = await updateConversation(user.id, id, input);
...
export const DELETE = withUser(async (user, _req: Request, ctx: Ctx) => {
  const { id } = await ctx.params;
  return await deleteConversation(user.id, id) ? ok({ deleted: id }) : fail("Conversation not found", 404);
});
